...
class Signup(APIView):
    ...
    def post(self, request):
        f_name = request.data.get('fname')
        l_name = request.data.get('lname')
        usr = request.data.get('uname')
        rl = request.data.get('role')
        try:
        ...
        user = User.objects.create(
            username=usr, first_name=f_name, last_name=l_name, is_staff=staff)
        user.set_password(psswd)
        user.save()
        sign = Customer.objects.create(
            status=sta, user=user, mobile=con, image=img, gender=gen, id_card_no=id1)
